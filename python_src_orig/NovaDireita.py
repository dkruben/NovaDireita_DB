...
def setup_logging():
    ...
    # Add a console handler to see logs in the console as well
...
class PrintPdf(FPDF):
    def header(self):
        self.image('icon/nova_direita.png', 10, 8, 33)
...
class Users(object):
    def __init__(self, idUser=0, name='', address='', city='', district='', cep='', phone='', nif='', cc='', aniversario=None, email='', available=''):
        logger.info(f"Creating new User object: {name}")
        self.info = {}
        self.idUser = idUser
        self.name = name
    ...
    def insert_militant(self):
        logger.info(f"Inserting militant into database: {self.name}")
        try:
            c = banco.conn.cursor()
            ...
            banco.conn.commit()
            logger.info(f"Militant {self.name} inserted successfully")
            # email
...
class Application:
    ...
    def openSecondWindow(self):
        ...
        # second_window.iconbitmap('icon/logo.ico')
    ...
    def insert_militant(self):
        ...
        if not user.phone.isdigit() or len(user.phone) != 9:
            self.lbl_msg['text'] = 'O número de telefone deve conter 9 dígitos.'
            return
        # Validate NIF
        if not user.nif.isdigit() or len(user.nif) != 9:
            self.lbl_msg['text'] = 'O NIF deve conter 9 dígitos.'
            return
        # Validate CC
        if not user.cc.isdigit() or len(user.cc) != 8:
        ...
        self.clear_fields()
        logger.debug(f"Militant data: {user.__dict__}")
        result = user.insert_militant()
    ...
    def change_militant(self):
        ...
        self.clear_fields()
        logger.debug(f"Updated militant data: {user.__dict__}")
        result = user.update_militant()
        logger.info(f"Update militant result: {result}")
    
    def exclude_militant(self):
        logger.info(f"Attempting to delete militant: {self.txt_idUser.get()}")
        user.idUser = self.txt_idUser.get()
        self.txt_idUser.delete(0, END)
        self.txt_name.delete(0, END)
        self.txt_address.delete(0, END)
        ...
        self.clear_fields()
        result = user.delete_militant()
        logger.info(f"Delete militant result: {result}")
    ...
    def search_militant(self):
        logger.info(f"Searching for militant: {self.txt_idUser.get()}")
        userId = user.idUser
        ...
        self.fill_fields()
        result = user.select_militant(idUser)
        logger.info(f"Search militant result: {result}")
...
if __name__ == '__main__':
    if os.name == 'nt':
        ...
        if not pyuac.isUserAdmin():
            ...
            logger.info("==================================")
            logger.info("Starting Nova Direita application")
            root = Tk()
            ...
            root.mainloop()
            logger.info("Nova Direita application closed")
