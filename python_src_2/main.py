...
from utils import create_district_city_parish_selection
...
class Application(wx.Frame):
    ...
    def add_input_fields(self, panel, vbox):
        ...
        self.name_text = wx.TextCtrl(panel, style=wx.TE_MULTILINE)
        vbox.Add(self.name_text, proportion=1, flag=wx.EXPAND | wx.ALL, border=5)
        # Morada
        ...
        self.district_combo.SetValidator(CityDistrictParishValidator())
        ...
        self.city_combo.SetValidator(CityDistrictParishValidator())
        ...
        self.bi_cc_text.SetHint("Formato: 12345678 1 ZA2")
        ...
        default_date = datetime.date(1960, 1, 1)
        self.birthday_picker.SetValue(wx.DateTime.FromDMY(default_date.day, default_date.month - 1, default_date.year))
        # Open Second Window
    ...
    def validate_form(self):
        fields = [
            self.name_text, self.address_text, self.parish_combo,
        ...
        if self.terms_radio.GetSelection() != 0:  # 0 is "Sim"
            ...
            return False
        birthday = self.birthday_picker.GetValue()
        today = wx.DateTime.Now()
        age = today.GetYear() - birthday.GetYear()
        if (today.GetMonth() < birthday.GetMonth()) or (today.GetMonth() == birthday.GetMonth() and today.GetDay() < birthday.GetDay()):
            age -= 1
        if age < 18:
            wx.MessageBox("Você deve ter 18 anos ou mais para se inscrever.", "Erro", wx.OK | wx.ICON_ERROR)
            return False
        return True
    ...
    def on_save(self, event):
        ...
        parish = self.parish_combo.GetValue()
        ...
        try:
            banc.c.execute("INSERT INTO militantes (name, address, district, city, parish, zip_code, phone, bi_cc, nif, email, birthday, agreement) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (name, address, district, city, parish, zip_code, phone, bi_cc, nif, email, birthday, terms))
            banc.conn.commit()
    ...
    def on_update(self, event):
        ...
        try:
            banc.c.execute("UPDATE militantes SET name=?, address=?, district=?, city=?, parish=?, zip_code=?, phone=?, bi_cc=?, nif=?, email=?, birthday=?, agreement=? WHERE militant=?", (name, address, district, city, parish, zip_code, phone, bi_cc, nif, email, birthday, terms, id_value))
            banc.conn.commit()
    ...
    def on_delete(self, event):
        id_value = self.id_text.GetValue()
        try:
            banc.c.execute("DELETE FROM militantes WHERE militant=?", (id_value,))
            banc.conn.commit()
            wx.MessageBox('Dados apagados com sucesso!', 'Info', wx.OK | wx.ICON_INFORMATION)
            self.on_clear()
        except sqlite3.Error as err:
            wx.MessageBox(f'Erro ao apagar dados: {err}', 'Erro', wx.OK | wx.ICON_ERROR)
            banc.connection.rollback()
    
    def on_search(self, event):
        search_militant = self.search_text.GetValue()
        try:
            banc.c.execute("SELECT * FROM militantes WHERE militant=?", (search_militant,))
            result = banc.c.fetchone()
            if result:
                self.populate_form_with_data(result)
            else:
                wx.MessageBox("Militante não encontrado.", "Resultado da pesquisa", wx.OK | wx.ICON_ERROR)
        except sqlite3.Error as err:
            wx.MessageBox(f"Erro ao pesquisar dados: {err}", "Erro", wx.OK | wx.ICON_ERROR)

    def populate_form_with_data(self, result):
        if result:
            militant, name, address, district, city, parish, zip_code, phone, bi_cc, nif, email, birthday, agreement = result
            self.id_text.SetValue(str(militant))
            self.name_text.SetValue(name)
            self.address_text.SetValue(address)
            self.parish_combo.SetValue(parish)
            ...
            if isinstance(birthday, datetime.date):
                self.birthday_picker.SetValue(wx.DateTime.FromDMY(birthday.day, birthday.month - 1, birthday.year))
            else:
                try:
                    year, month, day = map(int, birthday.split('-'))
                    if wx.DateTime.IsValid(wx.DateTime.FromDMY(day, month - 1, year)):
                    ...
                    wx.MessageBox("Erro ao processar a Data de Nascimento.", "Erro", wx.OK | wx.ICON_ERROR)
    ...
    def export_all_to_pdf(event):
        try:
            pdf = PrintPdf()
            pdf.add_page()
            banc.c.execute("SELECT * FROM militantes")
            rows = banc.c.fetchall()
            pdf.set_font('Arial', '', 10)
            column_names = ['Militante', 'Nome', 'Morada', 'Distrito', 'Cidade', 'Freguesia', 'Código Postal', 'Telefone', 'BI/CC', 'NIF', 'Email', 'Data de Nascimento', 'Aceita os Termos']
            for row in rows:
                pdf.chapter_title(f'Militante: {row[0]}')
                for i, field_value in enumerate(row):
                    field_name = column_names[i]
                    pdf.cell(0, 5, f'{field_name}: {field_value}', 0, 1)
                pdf.ln(10)
            if hasattr(banc, 'conn'):
                banc.conn.close()
            desktop_path = os.path.join(os.path.expanduser('~'), 'Desktop')
            export_folder = os.path.join(desktop_path, 'Nova Direita', 'PDF')
            os.makedirs(export_folder, exist_ok=True)  # Cria a pasta se não existir
            pdf_file = os.path.join(export_folder, 'Militantes.pdf')
            pdf.output(pdf_file)
            wx.MessageBox(f'PDF gerado com sucesso!', 'Info', wx.OK | wx.ICON_INFORMATION)
        except sqlite3.Error as err:
            wx.MessageBox(f'Erro ao exportar dados: {err}', 'Erro', wx.OK | wx.ICON_ERROR)
        except Exception as err:
            wx.MessageBox(f'Ocorreu um erro na criação do arquivo PDF: {err}', 'Erro', wx.OK | wx.ICON_ERROR)
    
    @staticmethod
    def export_all_to_xlsx(event):
        try:
            banc.c.execute("SELECT * FROM militantes")
            results = banc.c.fetchall()
            if not results:
                wx.MessageBox('Sem dados para exportar', 'Info', wx.OK | wx.ICON_INFORMATION)
                return
            desktop_path = os.path.join(os.path.expanduser('~'), 'Desktop')
            export_folder = os.path.join(desktop_path, 'Nova Direita', 'XLXS')
            os.makedirs(export_folder, exist_ok=True)  # Cria a pasta se não existir
            xlsx_file = os.path.join(export_folder, 'Militantes.xlsx')
            wb = openpyxl.Workbook()
            ws = wb.active
            headers = ['Militante', 'Nome', 'Morada', 'Distrito', 'Cidade', 'Freguesia', 'Código Postal', 'Telefone', 'BI/CC', 'NIF', 'Email', 'Data de Nascimento', 'Aceita os Termos']
            ws.append(headers)
            for row in results:
                ws.append(row)
            wb.save(xlsx_file)
            wx.MessageBox(f'Dados exportados com sucesso.', 'Info', wx.OK | wx.ICON_INFORMATION)
        except sqlite3.Error as e:
            wx.MessageBox(f'Erro ao exportar dados: {e}', 'Erro', wx.OK | wx.ICON_ERROR)
        except Exception as err:
            wx.MessageBox(f'Ocorreu um erro na exportação: {err}', 'Erro', wx.OK | wx.ICON_ERROR)
