...
from reportlab.platypus import Image, SimpleDocTemplate, Paragraph, Spacer, TableStyle, Table
from wx.adv import DatePickerCtrl

from constants import icon, email_body, db_name
from database import Database
from email_sender import EmailSender
from terms_conditions import SecondWindow
from utils import create_district_city_parish_selection, add_page_number, current_dir
...
class Application(wx.Frame):
    ...
    def add_input_fields(self, panel, vbox):
        ...
        # Certifique-se de que a função retorna três widgets
    ...
    def on_save(self, event):
        ...
        try:
            banc.cursor.execute("INSERT INTO militantes (name, address, district, city, parish, zip_code, phone, bi_cc, nif, email, birthday, agreement) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (name, address, district, city, parish, zip_code, phone, bi_cc, nif, email, birthday, terms))
    ...
    def export_all_to_pdf(event):
        try:
            ...
            # Definindo a página na orientação horizontal
            doc = SimpleDocTemplate(pdf_file, pagesize=landscape(A2))  # Usando A4 na orientação paisagem
            ...
            table_data = []
            headers = ['Militante', 'Nome Completo', 'Morada', 'Distrito', 'Cidade', 'Freguesia', 'Código Postal',
                       'Telefone', 'BI/CC', 'NIF', 'Email', 'Data de Nascimento', 'Aceita os Termos']
            table_data.append(headers)
            for row in results:
                table_data.append(row)
            militant_table = Table(table_data)
            style = TableStyle([
            ...
            militant_table.setStyle(style)
            elements.append(militant_table)
    ...
    def export_all_to_xlsx(event):
        try:
            ...
            ws = wb.active
            headers = ['Militante', 'Nome Completo', 'Morada', 'Distrito', 'Cidade', 'Freguesia', 'Código Postal', 'Telefone', 'BI/CC', 'NIF', 'Email', 'Data de Nascimento', 'Aceita os Termos']
            ws.append(headers)
            for row in results:
                ws.append(row)
            wb.save(xlsx_file)
...
if __name__ == '__main__':
    banc = Database(host='localhost', user='root', password='', database=db_name)
